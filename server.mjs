...
const sourcePages = [
  ...
  {
    name: "青眼情报行业资讯",
    url: "https://www.iqyqb.com/articleDetails?id=1003&types=1",
    category: "市场趋势",
    region: "domestic"
  },
  {
  ...
  {
    name: "化妆品财经在线",
    url: "https://www.cbo.cn/",
    category: "渠道动态",
    region: "domestic"
  },
  {
    name: "C2CC 传媒",
    url: "https://www.c2cc.cn/",
    category: "市场趋势",
    region: "domestic"
  },
  {
    name: "化妆品资讯",
    url: "https://www.cosmetic-news.net/",
    category: "市场趋势",
    region: "domestic"
  },
  {
    name: "国家药监局化妆品",
    url: "https://www.nmpa.gov.cn/xxgk/fgwj/bmgzh/20200331151901234.html",
    category: "法规政策",
    region: "domestic"
  },
  {
    name: "Jing Daily Beauty",
    url: "https://jingdaily.com/beauty/",
    category: "国际趋势",
    region: "global"
  },
  {
  ...
  {
    name: "Business Wire Cosmetics",
    url: "https://www.businesswire.com/newsroom/industry/retail/cosmetics",
    category: "品牌案例",
    region: "global"
  }
];
...
async function ensureDataFile() {
  ...
  if (!existsSync(aiConfigFile)) {
    await writeFile(aiConfigFile, JSON.stringify({
      aiEndpoint: process.env.AI_BASE_URL || "https://api.deepseek.com/chat/completions",
...
async function writeState(state) {
  await ensureDataFile();
  await writeFile(dataFile, JSON.stringify(normalizeState(state), null, 2));
}

async function readAiConfig() {
  await ensureDataFile();
  try {
    const saved = JSON.parse(await readFile(aiConfigFile, "utf8"));
    return {
      aiEndpoint: process.env.AI_BASE_URL || saved.aiEndpoint || "https://api.deepseek.com/chat/completions",
      aiModel: process.env.AI_MODEL || saved.aiModel || "deepseek-chat",
      aiToken: process.env.AI_API_KEY || saved.aiToken || ""
    };
  } catch {
    return {
      aiEndpoint: process.env.AI_BASE_URL || "https://api.deepseek.com/chat/completions",
...
async function writeAiConfig(config) {
  ...
  const next = {
    aiEndpoint: config.aiEndpoint || current.aiEndpoint,
    aiModel: config.aiModel || current.aiModel,
    aiToken: config.aiToken || current.aiToken
  };
  await writeFile(aiConfigFile, JSON.stringify(next, null, 2));
  return { aiEndpoint: next.aiEndpoint, aiModel: next.aiModel, hasToken: Boolean(next.aiToken) };
}
...
function normalizeState(state) {
  return {
    ...
    settings: {
      ...
      aiToken: state.settings?.aiToken || ""
    },
...
async function fetchText(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "user-agent": "CBE-learning-desk/1.0"
      }
    });
...
function parseFeed(xml, feed) {
  ...
  const region = typeof feed === "string" ? "global" : feed.region;
  ...
  return blocks.slice(0, 12).map((block) => {
    ...
    const link = cleanXml(readTag(block, "link")) || readAtomLink(block);
    const description = cleanXml(readTag(block, "description") || readTag(block, "summary") || readTag(block, "content:encoded"));
...
function isBeautyLike(text) {
  return /美妆|化妆品|护肤|彩妆|香水|香氛|洗护|功效|品牌|渠道|零售|门店|消费|备案|原料|成分|防晒|面膜|医美|个护|beauty|cosmetic|skincare|fragrance|retail|brand/i.test(text);
}
...
function classifyArticle(item) {
  ...
  if (/regulation|fda|nmpa|法规|监管|备案|功效/.test(text)) return "法规政策";
  if (/retail|channel|store|douyin|tmall|渠道|门店|零售|抖音|天猫/.test(text)) return "渠道动态";
  if (/brand|launch|loreal|shiseido|品牌|新品|上市/.test(text)) return "品牌案例";
  if (/consumer|gen z|消费|消费者|趋势/.test(text)) return "消费者洞察";
  return "市场趋势";
}

function stakeholderFor(item) {
  const text = `${item.title} ${item.summary}`.toLowerCase();
  if (/retail|channel|store|渠道|门店|零售/.test(text)) return "渠道商";
  if (/ingredient|supplier|packaging|原料|供应链|包材/.test(text)) return "供应链";
  if (/consumer|gen z|消费/.test(text)) return "消费者";
  return "品牌方";
}
...
async function syncNews({ includeGlobal = false } = {}) {
  ...
  const newItems = [...domesticItems, ...globalItems]
    .slice(0, 22)
    .map((item) => ({
      id: createId(),
      title: item.title,
...
async function pushCloud() {
  ...
  if (!state.settings.cloudEndpoint) {
    return { ok: false, message: "还没有配置云同步地址。" };
  }

  const response = await fetch(state.settings.cloudEndpoint, {
    method: "POST",
    headers: {
...
async function callAiProxy(body) {
  ...
  if (!config.aiEndpoint || !config.aiToken) {
    return {
      ok: false,
      message: "还没有配置本地 AI 中转接口。请先保存接口地址、模型和令牌。"
    };
  }
  ...
  const messages = Array.isArray(body.messages)
    ...
    : [
        {
          role: "system",
          content: "你是CBE日化销售新人助手。用中文解释日化、美妆、会展、渠道、合同续签和拓客相关问题，并给出销售可用表达。"
        },
        { role: "user", content: String(body.question || "") }
      ];

  const upstream = await fetch(config.aiEndpoint, {
    method: "POST",
    headers: {
      "content-type": "application/json",
  ...
  if (!upstream.ok) {
    return {
      ...
      message: data.error?.message || data.message || "AI 服务返回异常。"
    };
  }

  return {
    ok: true,
    answer: data.choices?.[0]?.message?.content || data.answer || data.content || ""
  };
}
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    if (requestUrl.pathname === "/api/health") {
      await sendJson(response, 200, { ok: true, mode: "server", port });
      return;
    }

    if (requestUrl.pathname === "/api/data" && request.method === "GET") {
      await sendJson(response, 200, await readState());
      return;
    }

    if (requestUrl.pathname === "/api/data" && request.method === "POST") {
    ...
    if (requestUrl.pathname === "/api/sync-news" && request.method === "POST") {
      await sendJson(response, 200, await syncNews({ includeGlobal: requestUrl.searchParams.get("includeGlobal") === "1" }));
      return;
    }

    if (requestUrl.pathname === "/api/reminders" && request.method === "GET") {
      const state = await readState();
      await sendJson(response, 200, { due: dueClients(state) });
      return;
    }

    if (requestUrl.pathname === "/api/cloud-push" && request.method === "POST") {
      await sendJson(response, 200, await pushCloud());
      return;
    }

    if (requestUrl.pathname === "/api/ai-config" && request.method === "GET") {
      const config = await readAiConfig();
      await sendJson(response, 200, {
        aiEndpoint: config.aiEndpoint,
        aiModel: config.aiModel,
        hasToken: Boolean(config.aiToken)
      });
      return;
    }

    if (requestUrl.pathname === "/api/ai-config" && request.method === "POST") {
      await sendJson(response, 200, await writeAiConfig(await readBody(request)));
      return;
    }

    if (requestUrl.pathname === "/api/ai" && request.method === "POST") {
      await sendJson(response, 200, await callAiProxy(await readBody(request)));
      return;
    }
...
server.listen(port, "127.0.0.1", () => {
  console.log(`CBE 日化销售工作台已启动：http://127.0.0.1:${port}`);
});
